// One-shot: extract → chunk → BATCH-embed → insert for R13_GL248 in FATF workspace.
// Uses Gemini batch embedding (up to 100 texts per API call) to dodge rate limits.
...
const FILE_PATH = "/Users/jeremyteh/Downloads/RHB Use Case 1,2,3/Use Case 3- New regulatory requirement/R13_GL248.docx";
const WORKSPACE = "fatf";
const TITLE_HINT = "R13 GL248";

const BATCH_SIZE = 50; // texts per Gemini call
...
// 1. Find target SOP
const { data: sops } = await supabase.from("sop_documents")
  .select("id,title,workspace_id").eq("workspace_id", WORKSPACE).ilike("title", `%${TITLE_HINT}%`);
const sop = sops[0];
...
console.log(`${chunks.length} chunks · batching ${BATCH_SIZE} per API call = ${Math.ceil(chunks.length/BATCH_SIZE)} calls`);
