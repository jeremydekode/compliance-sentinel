// MSIC 2008 (Malaysia Standard Industrial Classification) code list.
//
// Provenance: the same reference table used by the HLAP finance-automation
// project (prisma/data/codes/msic-codes.json). Stored as [code, label] tuples
// to keep the payload small — it is lazy-loaded by the review form, never part
// of the initial bundle.
//
// This exists so a filer never has to go and look a code up: the extracted
// nature-of-business text is matched against these labels and the result is
// offered for confirmation. Matching is exact/lexical, never model-generated —
// an invented MSIC code would be a false statutory declaration.

/** [code, label] — MSIC 2008, 5-digit. */
export const MSIC_CODES: ReadonlyArray<readonly [string, string]> = [
